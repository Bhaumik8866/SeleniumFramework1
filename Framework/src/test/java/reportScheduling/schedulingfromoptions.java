...
public class schedulingfromoptions extends initializeDriver{
	...
	@Test
	public void schedulingfromoptionsicon() throws IOException, InterruptedException
	{
		driver=driverinit();
		...
		for(int i=0;i< reportname.size();i++)
		{
			...
			{
			rs.xlsxoptionicon(reportname.get(i)).click();
			}else
			{
			rs.scheduleoutputoption(reportname.get(i), format).click();
			}
			...
			sch.outputname().sendKeys(paramdata.get(1)+" "+format);
			Assert.assertEquals(sch.outputformat().getText(),format);
			Thread.sleep(1000);
			sch.runbutton().click();
			Thread.sleep(5000);
			...
			{
			rs.xlsxoptionicon(reportname.get(i)).click();
			}else if(format.equalsIgnoreCase("phtml"))
			{
			rs.phtmloptionicon(reportname.get(i)).click();
			}else if(format.equalsIgnoreCase("html"))
			{
			rs.htmloptionicon(reportname.get(i)).click();
			}else
			{
			rs.scheduleoutputoption(reportname.get(i), format).click();
			}
...
//				requestsection.deleteallscheduledreports(driver);
				...
				{
					ArrayList<String> paramdata= param.getdata(reportname.get(i));
					ArrayList<String> list= reportformat.getdata(reportname.get(i));
					String[] arrayoutputformats=list.get(1).split(",");
					for(String format:arrayoutputformats)
					{
					requestsection.completedreportname(paramdata.get(1)+" "+format).click();
